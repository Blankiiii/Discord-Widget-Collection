...
function validateConfig(config) {
  ...
  if (!config.Player_Name || typeof config.Player_Name !== 'string') {
    throw new Error('config.json must include a valid Player_Name');
  }
  if (!VALID_PLATFORM_TYPES.includes(String(config.Platform_Type).toLowerCase())) {
    throw new Error(`Platform_Type must be one of: ${VALID_PLATFORM_TYPES.join(', ')}`);
  }
  if (!VALID_PLATFORM_FAMILIES.includes(String(config.Platform_Family).toLowerCase())) {
...
function getRankImageUrl(rankName, iconsConfig) {
...
function buildOutput(profile, operators, config) {
  ...
  const dynamic = [
    { type: 1, name: 'user_Name', value: config.Player_Name },
    { type: 1, name: 'user_Platform', value: mapPlatform(config.Platform_Type) },
    { type: 1, name: 'current_Rank', value: formattedCurrentRank }
  ];

  const currentRankUrl = getRankImageUrl(formattedCurrentRank, config.iconsConfig);
  if (currentRankUrl) {
  ...
  const maxRankUrl = getRankImageUrl(formattedMaxRank, config.iconsConfig);
  if (maxRankUrl) {
...
async function fetchRawR6Json(config) {
  validateConfig(config);

  const statsUrl = new URL('https://api.r6data.com/api/stats');
  statsUrl.searchParams.set('type', 'stats');
  statsUrl.searchParams.set('nameOnPlatform', config.Player_Name);
  statsUrl.searchParams.set('platformType', config.Platform_Type);
  statsUrl.searchParams.set('platform_families', config.Platform_Family);
  ...
  operatorStatsUrl.searchParams.set('nameOnPlatform', config.Player_Name);
  operatorStatsUrl.searchParams.set('platformType', config.Platform_Type);
  operatorStatsUrl.searchParams.set('platform_families', config.Platform_Family);
  ...
  if (!statsResp.ok) {
    ...
    throw new Error(`R6Data stats API error ${statsResp.status}: ${text}`);
  }

  if (!opsResp.ok) {
    const text = await opsResp.text();
    throw new Error(`R6Data operatorStats API error ${opsResp.status}: ${text}`);
  }
  ...
  const rankedProfile =
    searchObject(statsData, (item) => item && typeof item === 'object' && item.board_id === 'ranked' && Array.isArray(item.full_profiles)) ||
    searchObject(statsData, (item) => item && typeof item === 'object' && Array.isArray(item.full_profiles) && item.full_profiles.some((p) => p && p.profile));

  const profile = rankedProfile?.full_profiles?.[0]?.profile || rankedProfile?.profile || statsData;
  return buildOutput(profile, operators, config);
}

module.exports = { fetchRawR6Json };
